...
# -*- coding: utf-8 -*-
"""
IEEE-style plotter: Entry index vs Memory (MiB) with shaded envelopes.

- X-axis is row index (1..N); time columns are ignored.
- Robust CSV parsing (delimiter sniff, unit stripping, decimal commas).
- Auto-detect Y column or use --ycol (case-insensitive).
- Optional smoothing in samples (centered moving average).
- Optional extension to a common number of entries (linear/hold).
- Per-series shaded band using rolling window (mean ± scale·std).
- Sparse, distinct markers; saves high-res PNG + vector PDF.
